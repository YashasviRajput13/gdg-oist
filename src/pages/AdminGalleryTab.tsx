...
const AdminGalleryTab = () => {
  ...
  const fetchItems = async () => {
    const { data } = await supabase.from("gallery_items").select("id, src, alt, caption").order("created_at", { ascending: true });
    if (data) setItems(data);
    setLoading(false);
  };

  const handleAdd = async () => {
    if (!form.src.trim() || !form.alt.trim()) { toast({ title: "Required", description: "Image URL and alt text are required.", variant: "destructive" }); return; }
    const { error } = await supabase.from("gallery_items").insert({ src: form.src.trim(), alt: form.alt.trim(), caption: form.caption.trim() || null });
    if (error) { toast({ title: "Error", description: error.message, variant: "destructive" }); return; }
    toast({ title: "Added!" }); setForm({ src: "", alt: "", caption: "" }); setIsAdding(false); fetchItems();
  };

  const handleUpdate = async (id: string) => {
    if (!form.src.trim() || !form.alt.trim()) { toast({ title: "Required", description: "Image URL and alt text are required.", variant: "destructive" }); return; }
    const { error } = await supabase.from("gallery_items").update({ src: form.src.trim(), alt: form.alt.trim(), caption: form.caption.trim() || null }).eq("id", id);
    if (error) { toast({ title: "Error", description: error.message, variant: "destructive" }); return; }
    toast({ title: "Updated!" }); setEditingId(null); setForm({ src: "", alt: "", caption: "" }); fetchItems();
  };
  ...
  const startEdit = (item: GalleryItem) => { setEditingId(item.id); setForm({ src: item.src, alt: item.alt, caption: item.caption || "" }); setIsAdding(false); };
  const cancelEdit = () => { setEditingId(null); setIsAdding(false); setForm({ src: "", alt: "", caption: "" }); };
    ...
    <div>
      ...
      {isAdding && (
        <Card className="mb-6 border-primary/30">
          <CardHeader className="pb-3"><CardTitle className="text-base">New Image</CardTitle></CardHeader>
