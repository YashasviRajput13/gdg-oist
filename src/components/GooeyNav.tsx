...
const GooeyNav = ({
  ...
  useEffect(() => {
    ...
    const activeLi = navRef.current.querySelectorAll('li')[internalActive];
    if (activeLi) {
      updateEffectPosition(activeLi);
      textRef.current?.classList.add('active');
    }
    ...
    const resizeObserver = new ResizeObserver(() => {
      const currentActiveLi = navRef.current?.querySelectorAll('li')[internalActive];
      ...
      <nav>
        <ul ref={navRef}>
          {items.map((item, index) => (
            <li key={index} className={internalActive === index ? 'active' : ''}>
              <a
                href={item.href}
